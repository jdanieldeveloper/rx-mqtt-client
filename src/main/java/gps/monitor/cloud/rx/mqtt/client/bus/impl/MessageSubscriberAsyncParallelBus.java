 ...
 */
public class MessageSubscriberAsyncParallelBus implements Bus {

    private static MessageSubscriberAsyncParallelBus mqttSubscriberBus;

    private static WorkQueueProcessor<Object> processor = WorkQueueProcessor.create();

    private static final Logger logger = LoggerFactory.getLogger(MessageSubscriberAsyncParallelBus.class);
     ...
     */
    public static MessageSubscriberAsyncParallelBus getInstance() {
        if (mqttSubscriberBus == null) {
            mqttSubscriberBus = new MessageSubscriberAsyncParallelBus();
        }
            ...
            .publish()
             ...
             .parallel()
                .subscribe(subscriber, e -> {
                    logger.error("[{}] Se ha producido un error en el flujo del subcriptor paralelo asincronico!!!", MessagePublisherBus.class.getSimpleName());
